...
for m in glob.glob('./annotacje_bin_bin_bin_bin.*'):
    "print(m)"
...
df2.to_csv('merged_files.csv', index=False)
   ...
   # obcięcie nazwy binu do jego numeru

    # utworzenie jednego pliku csv dla wszystkich binów
    #for bin_number in range(len(p_dict)):

    #df.to_csv(f'biny_metaflye{m}.csv')
